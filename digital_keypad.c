...
unsigned char read_digital_keypad(unsigned char mode)
{
    ...
    /* If key released in short time return same key values */
    ...
    /* Increment long press value */
	else if (once && longpressed <= 30 )
		longpressed++;
    
    /* If Switch 4 is long pressed return LPSW4 */
	else if ( once && longpressed == 31  && key == SW4)
	{ 
		longpressed ++;
		return  LPSW4;
	}
    /* If Switch 5 is long pressed return LPSW5 */
